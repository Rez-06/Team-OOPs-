...
void Register::cashOut(double amount, string reason) {
    ...
    if (reason.empty()) reason = "Cash out";
...
void Register::showIncomeSummary() const {
    ...
    for (const auto& tx : history) {
        if (tx.type == "FOOD") foodIncome += tx.amount;
        else if (tx.type == "RENT") rentIncome += tx.amount;
        else if (tx.type == "CASHOUT") cashoutTotal += (-tx.amount); // tx.amount negative
    }
    cout << "\n====== Income Summary ======\n";
